...
- **No dual-representation.** When an API response includes a dedicated structured field for a content type (`table: Row[]`, `chart: ChartSpec`, `code: string`), that content must **not** also be embedded in the prose field. Each piece of output lives in exactly one field — prose fields contain only prose. Violating this causes the same content to render twice: once as raw markdown syntax inside the text bubble, and once as the properly-rendered structured component below it.
