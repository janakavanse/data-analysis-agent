...
`git commit -m "..." && git push origin <branch>`. A commit that is not pushed does
not exist. A hook enforces this (`.claude/hooks/`); do not rely on memory.

## Branches & PRs

- Application code lives on a feature branch: `git checkout -b feature/<slug>`.
...
- Only harness / spec / boilerplate changes may be committed directly to `main`.
- If you find yourself on `main` about to write application code, stop and branch.
...
- Commit message format: `phase-N: <what you did>` for build work; `fix: …`,
  `spec: …`, `harness: …` otherwise.
