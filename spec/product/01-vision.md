...
- **No file formats other than CSV** in the first release (no JSON, Excel, Parquet, database
  connectors).
- **No charts or visualizations** — the answer is text + a result table only.
...
- **In-memory analytical engine.** DuckDB holds the per-dataset tables; data does not survive a
  process restart unless re-materialized from stored files (see [`04-data-model.md`](04-data-model.md)
  § Data Lifecycle).
...
- **Charts / visualizations** of result data.
